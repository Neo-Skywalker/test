...
class Runner:
    def __init__(self, conf_path, mode='render', case='CASE_NAME', is_continue=False):
        ...
        self.load_checkpoint(r'/home/yjh/NeRF/nerf_model.pth', absolute=True)
    ...
    def save(self):
        ...
        for batch in tqdm(range(0, pts_xyz.shape[0], batch_size)):
            ...
            net_sigma, net_color = self.fine_nerf(batch_pts_xyz.to(self.device), torch.zeros_like(batch_pts_xyz).to(self.device))
            sigma[batch:batch+batch_size] = net_sigma.detach().cpu()
            color[batch:batch+batch_size] = net_color.detach().cpu()
        # TODO
        # with open('sigma.json','w') as f:
        #     json.dump({'data':sigma.reshape(512).tolist()},f)
        # print(f'\nI have finished writting rays_o,shape:{sigma.shape}')
        self.my_nerf.save(pts_xyz, sigma, color)

    def render_video(self):
        images = []
        resolution_level = RS_LVL
        n_frames = 90
        for idx in tqdm(range(n_frames)):
            rays_o, rays_d = self.dataset.gen_rays_at(idx, resolution_level = resolution_level)
            # with open('rays_o.json','w') as f:
            #     json.dump({'data':rays_o.tolist()},f)
            # print(f'\nI have finished writting rays_o,shape:{rays_o.shape}')
            H, W, _ = rays_o.shape
            ...
            for rays_o_batch, rays_d_batch in tqdm(zip(rays_o, rays_d)):
                ...
                ######
                # with open('render_out.json','w') as f:
                #     json.dump(render_out,f)
                #     a = render_out['z_vals']
                #     print(f'\nI have finished writting renderout,zvals shape:{len(a)}')
                #     sys.exit()
                ######
